...
const RECIPES = [
  ...
  // The real Surf & Turf sits on chilled ramen noodles and fresh greens, not
  // rice (the greens live in the base here, so they're out of the mix-ins).
...
// How each base paints its bed: the solid under-color, how many morsels
// cover it, and how big they run.
const BED = {
...
// A full bed of the base(s) covering the whole opening. Two bases (the Surf
// & Turf's noodles + greens) interleave so the bed reads as a mix.
function drawBed(cx, rimY, innerRx, innerRy, baseNames) {
  ...
  // Average the coverage across the mix so a rice+salad bowl isn't denser
  // than either alone.
